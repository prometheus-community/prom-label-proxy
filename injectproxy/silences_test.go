...
func getSilenceWithoutLabel() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		if req.Method != "GET" {
			prometheusAPIError(w, req,(w, "invalid method: "+req.Method, http.StatusInternalServerError)
			return
		}
		if req.URL.Path != "/api/v2/silence/"+silID {
			prometheusAPIError(w, req,(w, "invalid path: "+req.URL.Path, http.StatusInternalServerError)
			return
		}
...
func createSilenceWithLabel(labelv string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if len(values) != 1 {
			prometheusAPIError(w, req,(w, fmt.Sprintf("expected 1 matcher for label %s, got %d", proxyLabel, len(values)), http.StatusInternalServerError)
			return
		}
		if values[0] != labelv {
			prometheusAPIError(w, req,(w, fmt.Sprintf("expected matcher for label %s to be %q, got %q", proxyLabel, labelv, values[0]), http.StatusInternalServerError)
			return
		}
...
func (c *chainedHandlers) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	if c.idx >= len(c.handlers) {
		prometheusAPIError(w, req,(w, "", http.StatusInternalServerError)
		return
	}
