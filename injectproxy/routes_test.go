...
func checkParameterAbsent(param string, next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			prometheusAPIError(w, req,(w, fmt.Sprintf("unexpected error: %v", err), http.StatusInternalServerError)
			return
		}

		if len(kvs[param]) != 0 {
			prometheusAPIError(w, req,(w, fmt.Sprintf("unexpected parameter %q", param), http.StatusInternalServerError)
			return
		}
...
func checkFormParameterAbsent(param string, next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		kvs := req.Form
		if len(kvs[param]) != 0 {
			prometheusAPIError(w, req,(w, fmt.Sprintf("unexpected Form parameter %q", param), http.StatusInternalServerError)
			return
		}
		next.ServeHTTP(w, req)
	})
...
func checkQueryHandler(body, key string, values ...string) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if len(kvs[key]) != len(values) {
			prometheusAPIError(w, req,(w, fmt.Sprintf("expected %d values of parameter %q, got %d", len(values), key, len(kvs[key])), http.StatusInternalServerError)
			return
		}
		...
		for i := range values {
			if kvs[key][i] != values[i] {
				prometheusAPIError(w, req,(w, fmt.Sprintf("expected parameter %q with value %q, got %q", key, values[i], kvs[key][i]), http.StatusInternalServerError)
				return
			}
		}
		...
		if err != nil {
			prometheusAPIError(w, req,(w, "failed to read body", http.StatusInternalServerError)
			return
		}

		if string(buf) != body {
			prometheusAPIError(w, req,(w, fmt.Sprintf("expected body %q, got %q", body, string(buf)), http.StatusInternalServerError)
			return
		}
